...
int ledPin = 7;                   // pin for the red LED
int ledPin1 = 8; 
...
void loop()
{
  ...
  {     
    // If a message has been received, do this
     data = Serial.read();  
    // Control LED 1 connected to pin 13
    // Get the received character
    if (data == '1') // If the read character is equal to 1
    {
      digitalWrite(ledPin1, HIGH); // Turn on the LED
      Serial.write('1'); // Send the new state of the LED  
    }  
  }
...
{    
  ...
  // Check if BPM is more than 60
  if (BPM > 90) {
    // Buzz for 3 seconds
    digitalWrite(buzzerPin, HIGH);
    digitalWrite(ledPin, HIGH); // Turn on the red LED
    Serial.println("BuzzerActive"); // Send message to indicate buzzer activation
    delay(3000);
